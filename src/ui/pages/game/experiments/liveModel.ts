...
/** 独立实验显示管线：仅一个在途任务，不持有可回收 VM 缓冲，不积压历史帧。 */
export class LiveModel {
  ...
  async load(file: File, modelId: LiveModelId = 'ultra4x-fp16'): Promise<void> {
    if (!isLiveModelId(modelId)) throw new Error('该模型尚未开放整帧实验');
    this.modelName = PROBE_MODELS.find((model) => model.id === modelId)!.name;
    if (file.size < 1 || file.size > 40 * 1024 * 1024) throw new Error('请选择已登记的整帧实验 ONNX');
    const model = await file.arrayBuffer();
    if (this.dead) throw new Error('实验已取消');
    await new Promise<void>((resolve, reject) => {
      ...
      this.deadline();
      this.worker.postMessage({ type: 'load', modelId, model }, [model]);
    });
  }
  private deadline() {
    clearTimeout(this.timer);
    this.timer = setTimeout(() => this.fail('超过 120 秒，已停止并恢复原始画面'), 120_000);
  }
  ...
  get status(): string {
    if (this.failure) return `${this.modelName} 整帧实验失败：${this.failure}`;
    if (!this.ready) return `${this.modelName} 整帧实验：正在加载`;
    if (!this.output) return `${this.modelName} 整帧实验：首帧推理中，暂显示原图`;
    return `${this.modelName} ${this.inputWidth}×${this.inputHeight} → ${this.output.width}×${this.output.height} · 推理/回读 ${this.inference.toFixed(1)}ms · 采样到结果 ${this.elapsed.toFixed(1)}ms · 当前画面年龄 ${Math.max(0, performance.now() - this.displayedAt).toFixed(0)}ms（非输入延迟）`;
  }
  /** 主线程只在空闲时复制最新整帧；鼠标重绘不重跑同一张 VM 帧。 */
  frame(frame: VmFrame): VmFrame | null {
    if (this.dead) return null;
    if (frame.width > 800 || frame.height > 600) {
      this.fail('整帧实验只支持不超过 800×600，请先降低游戏分辨率');
      return null;
    }
  ...
  destroy(): void {
    ...
    this.output = null;
    this.source = null;
    this.rejectLoad?.(new Error(this.failure || '实验已取消'));
    this.rejectLoad = undefined;
  }
}
